...
for k in range(n_splits):
    ...
    savepath = f'/cofast/lhoffman/slowdown/D2_cesm2le_sst_seasonal_robustness_split{k}_TVT_1990-2040.nc'
    ds = xr.Dataset(
        {
...
for k in range(n_splits):
    ...
    sie_tri = siei[:,train_blocks,:]
    sie_vai = siei[:,val_block,:]
    sie_tei = siei[:,test_block,:]
    ...
    time_tei = timet[:,test_block,:]
    time_vai = timet[:,val_block,:]
    time_tri = timet[:,train_blocks,:]
    ...
    savepath = f'/cofast/lhoffman/slowdown/D2_cnn_cesm2le_sieTREND_seasons_robustness_split{k}_TVT_1990-2100.nc'
    ds = xr.Dataset(
        {
...
for k in range(n_splits):
    ...
    savepath = f'/cofast/lhoffman/slowdown/D2_cnn_cesm2le_siextent_seasons_robustness_split{k}_TVT_1990-2100.nc'
    ds = xr.Dataset(
        {
            "sie_tr":(("ntr",),sie_tr),
            "sie_va":(("nva",),sie_va),
