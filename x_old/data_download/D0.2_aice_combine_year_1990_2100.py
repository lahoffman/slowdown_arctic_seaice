...
loadpath = '/cofast/lhoffman/cesmle/aice/year/'
...
for i in range(50):
    for j in range(12):
        if j < 3:
            filepath = '/cofast/lhoffman/cesmle/aice/year/BHISTcmip6_year_{}_aice_{}01-{}12.nc'.format(cmip6_members[i],start_years[j],end_years[j])
        else: 
            filepath = '/cofast/lhoffman/cesmle/aice/year/BSSP370cmip6_year_{}_aice_{}01-{}12.nc'.format(cmip6_members[i],start_years[j],end_years[j])
        
        dataset = nc.Dataset(filepath,'r')
        aice_yr = np.array(dataset.variables['aice_yearly'])
        years = np.array(dataset.variables['unique_years'])
        
        if j == 0:
            aicej = aice_yr
            yearsj = years
        else:
            aicej = np.concatenate((aicej,aice_yr),axis=0)
            yearsj = np.concatenate((yearsj,years),axis=0)
    aicei.append(aicej)
    yearsi.append(yearsj)
aice = np.array(aicei)
time = np.array(yearsi)[0,:]
...
savepath = '/cofast/lhoffman/cesmle/aice/year_combined/aice_cesmle_first50members_year_190001-210012.nc'

# Create an xarray Dataset
ds = xr.Dataset(
    {
        "aice": (("nem", "nm", "nx", "ny"), aice),
        "time": (("nm",), time),
        "lat": (("nx","ny"), lat),
        "lon": (("nx","ny"), lon),
    },
    coords={
        "nem": np.arange(aice.shape[0]),  # Ensemble members index
        "nm": np.arange(aice.shape[1]), #years
        "nx": np.arange(aice.shape[2]),  # Latitude index
        "ny": np.arange(aice.shape[3]),  # Longitude index
    },
)
...
for i in range(50):
    for j in range(12):
        if j < 3:
            filepath = '/cofast/lhoffman/cesmle/aice/year/BHISTsmbb_year_{}_aice_{}01-{}12.nc'.format(smbb_members[i],start_years[j],end_years[j])
        else: 
            filepath = '/cofast/lhoffman/cesmle/aice/year/BSSP370smbb_year_{}_aice_{}01-{}12.nc'.format(smbb_members[i],start_years[j],end_years[j])
...
savepath = '/cofast/lhoffman/cesmle/aice/year_combined/aice_cesmle_last50members_year_190001-210012.nc'
