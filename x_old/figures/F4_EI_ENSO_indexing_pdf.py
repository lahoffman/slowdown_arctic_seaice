...
for k in range(4,5):
    for r_idx in range(5):
        ...
        y_predi = y_pred[:4000,]
        nino_i = nino_tr[:4000]
        nino_i_labels = nino_tr_labels[:4000]
    ...
    def event_dependence_by_phase(phase_labels, event_bool, phases=(-1,0,1), B=5000):
        ...
        for i, v in enumerate(phases):
            m = (ph == v) # index where phase_label (data) = phase (-1,0,1)
            if np.any(m):
                counts[i, 0] = np.sum(~evt[m])     # no [evt where data in phase, evt ~= 0, no slowdown + phase]
                counts[i, 1] = np.sum( evt[m])     # yes
                p_event[i]   = counts[i, 1] / counts[i].sum()  #yes slowdowns + phase / total phase
    ...
    # ---------------------------------------
    def plot_overlay_and_stacked(res_slow, res_tp, labels=("La Niña","Neutral","El Niño")):
        ...
        ax.set_ylabel("P(event | ENSO phase)")
        ax.set_title("Conditional probability by ENSO phase")
        ...
        ax.set_xticks(x); ax.set_xticklabels(labels)
        ax.set_ylabel("P(slowdown | ENSO phase)")
        ax.set_title("Predictable share of slowdowns by ENSO phase")
        ax.legend(frameon=False)
    ...
    def logit_or_r2(enso_labels, y_binary):
        """Odds ratio per +1 ENSO step + McFadden R^2 (small but informative)."""
    ...
    print("\n=== Logistic regression (per +1 ENSO step) ===")
    ...
    def plot_tp_conditional(boot_tp, phases=(-1,0,1), labels=("La Niña","Neutral","El Niño")):
        ...
        # ---- Print results ----
        print("\nConditional probability of CNN true-positive slowdowns (P(TP|ENSO)):")
        ...
        ax.set_xticks(x); ax.set_xticklabels(labels)
        ax.set_ylabel("P(TP slowdown | ENSO phase)")
    ...
    def variance_explained_by_phase(phase_labels, event_bool, phases=(-1,0,1), B=5000):
        """
        Returns % variance explained by ENSO (between-phase / total) for a binary event.
    ...
    print("Slowdowns: VE_ENSO = {:.3%}  (95% CI: [{:.3%}, {:.3%}])".format(
    ...
    print("TP slowdowns: VE_ENSO = {:.3%}  (95% CI: [{:.3%}, {:.3%}])".format(
