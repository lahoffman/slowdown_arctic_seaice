...
- [~] 8.6 Interannual sanity check + onset-year-in-window test (`09_interannual_check.py`): *code done; run on profx.* Read-out: R² of trend on SIE(t) with the window starting at t+1 — white noise gives 0.25 at offset 0 and 0 at offset 1, so the paper's 0.28 is mostly arithmetic unless offset 1 holds up.
