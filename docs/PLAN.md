...
- [x] **OHC done** (`run_ohc.sh`): extraction cmip6 OHC100/300 monthly; LB22 test → OHC map keeps R² 0.086 / AUROC 0.685 at offset vs GMT state 0.023/0.591 (¼ real, ¾ coupling); SIE ledger → no OHC predictor adds > 0.013 beyond ice state (IPO +0.025). See REVISION_PLAN 09-16 block. Was: **OHC (tomorrow, one screen): `scripts/run_ohc.sh`** = extract OHC100+300 (cmip6, ~7 h) → `09_ohc_lb22_test.py` (their predictor + GMT-state baseline + offset; also for SIE) → `09_ohc_sie_ledger.py --depth 100|300` (Barents–Kara & co. at onset → residual after ice state; corr map). Smoke test done: 8 min/member; store has no smbb historical → cmip6 only. Read `results/ohc/*/summary.md`. Was: OHC from CESM2-LE top levels only (`docs/OHC_PLAN.md`): catalog check (AWS Zarr `ncar-cesm2-lens`, else THREDDS/OPeNDAP hyperslabs) → annual OHC100/OHC300 → (a) LB22 test with *their* predictor + GMT-state baseline, (b) 8.5 Barents–Kara heat in the SIE ledger. Decide in/after this paper once the catalog check says how heavy it is.
- [ ] Main-text figure plan (agreed): 1 mechanism (definition + Eq. + GMT/SIE bars) · 2 XAI coherence (composite + methods + occlusion) · 3 skill vs baselines both label sets · 4 the extra-Arctic test (off1_pacific) · 5 ledger · 6 observations. Interannual, learning curves/frozen scalar, thickness, RILE, regression → SI.
