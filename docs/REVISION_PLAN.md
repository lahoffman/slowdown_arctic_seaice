...
**Step 8.6 — Interannual sanity check and the onset-year-in-window test (`scripts/09_interannual_check.py`, `src/analysis/interannual.py`; minutes).** (a) Per-member and 30-yr running correlations of the September SIE anomaly with JJA Niño 3.4 / IPO / Arctic SST, and test R² of SIE(t) ~ SIE(t−1) with and without the concurrent indices: does CESM2-LE carry the year-to-year Pacific bridge at all, and how nonstationary is it (B&BW 2020 showed this for CESM1-LE; Wyburn-Powell et al. 2024 across ensembles) — one SI panel, not a result. (b) **The arithmetic check.** The onset year sits at the start of the fitted trend window, so a low SIE(t) mechanically flattens the slope from t. For a *white-noise* series this alone gives corr(slope, y₀) = x₀/√Σx² = −0.50, i.e. R² ≈ 0.25 — the synthetic test reproduces it exactly (0.255 at offset 0, 0.000 at offset 1). The paper's r = −0.53 / R² 0.28 is therefore mostly arithmetic unless the offset-1 window (t+1…t+10) retains a substantial R². *Read-out:* R²(offset 1) is the genuine ice-state memory; if it is ≲ 0.05 the "ice state dictates slowdowns" sentence becomes "the label is correlated with SIE(t) by construction plus a small persistence term", and the logistic-SIE baseline's strength — and the CNN's Arctic reading — are largely the same artefact. LB22's GMST-at-onset has the identical property. Figure `diagnostics/interannual_check.png` (a–e).

**Step 8.7 — CNN regression head (`scripts/04_cesm2le_cnn_regress.py`, `build_cnn(task='regression')`, Huber loss).** Continuous target: the trend anomaly, standardised with training statistics; inputs from any classification tag's split files; scored as test R² against OLS on SIE and on SIE + Niño 3.4 + IPO fitted on the same samples. Threshold-free version of Phase 1. Subset first (`--splits 2 5 7 --n-runs 2`). Outputs `results/regression/<tag>/regress_summary.md`, figure `diagnostics/regression_<tag>.png`. If 8.6(b) shows the arithmetic dominates, rerun with the offset-1 target (to add: `--trend-offset 1`).

**Step 8.8 — BUG FOUND (2026-09-14): the `aux` scalar was effectively frozen; `rel_aux` was not a fair test.** Synthetic check: label depending *only* on the scalar (oracle AUROC 0.84) → aux-CNN reaches 0.62; the scalar's output weight moves from −0.01 to +0.05 in 35 epochs. Cause: Adam at lr 1e-4 changes a weight by ~lr per step; ~28 steps/epoch × 10–30 epochs ≈ 0.03–0.08 total, from a Glorot init of ±0.05 with fan-in ~1000 map features. So `rel_aux` ≈ `rel_base` and `rel_aux` < `logit_sie_anom` are optimisation artefacts, not evidence about the map. `rel_base`, occlusion and all linear baselines are unaffected. **Fix:** `build_cnn(aux_init=(coef, intercept))` warm-starts the output layer at the training-set logistic (or OLS) fit on the scalars with zero map weights, so training starts *at* the scalar baseline (synthetic: 0.837 at init → 0.836 trained, weight preserved). `04 --no-warm-start` reproduces the old behaviour. **Consequence:** retrain the aux configurations — `rel_aux` → `rel_aux_ws` (same splits, symlink `tvt_splits/rel_aux_ws → rel_aux`), and `rel_openwater`, `rel_lag1`, `rel_concurrent` likewise once `rel_aux_ws` is read. The Branch-A/B gate is re-opened for exactly one question: with a fair aux input, does the map lift AUROC above `logit_sie_anom` (0.765) toward `logit_sie_pacific` (0.80)? The `rel_base` result (0.72 ≈ `logit_indices`) already bounds what the map alone contains, so the expectation is still ≤ 0.80.

```
ln -s $SLOWDOWN_DATA_ROOT/results/tvt_splits/rel_aux $SLOWDOWN_DATA_ROOT/results/tvt_splits/rel_aux_ws
python -u scripts/04_cesm2le_cnn_train.py --tag rel_aux_ws --splits 2 5 7 --n-runs 2        # subset: is it now ≥ 0.765?
scripts/run_retrain.sh rel_aux_ws && scripts/run_postprocess.sh rel_aux_ws                   # full, if the subset says so
...
**Steps 8.5 + 8.9 results (2026-09-14, offset labels, `results/residual/residual_summary.md`).** Trend anomaly over t+1…t+10 regressed on the onset state, test R² (median over 9 splits): SIE 0.075 (range 0.003–0.104); SEP volume alone 0.078, MAR volume alone 0.067; SIE + volume 0.087 (either month). Residual on indices at onset: IPO +0.027, Pacific +0.029 (−0.005 with volume in stage 1), Niño 3.4 +0.005, Arctic SST +0.002; all concurrent (t+1…t+10 mean) indices ≈ 0. Maps: onset SST shows the faint positive-IPO pattern (|r| ≲ 0.1) with shallower subsequent decline; the Arctic has vanished from both maps. **Ledger: ≈ 0.11 of the following decade's trend-anomaly variance is foreseeable from anything at onset (ice 0.075 + volume 0.012 + IPO 0.024); ≈ 0.89 is not.** The "volume carries the memory" hypothesis is not supported for this target (extent and volume are too collinear at pan-Arctic scale; consistent with thickness helping *state* prediction at 1–3 yr, not decadal trends).

**Venue decision (2026-09-14): AIES.** The paper is an audit — onset-inclusive label artefact (transferable to LB22 and any trend-slowdown definition), persistence baselines before XAI, LRP association ≠ skill (ours, LB22, Li et al. 2026), frozen scalar input, early-stopping on noisy validation — with the CESM2-LE decadal-slowdown problem as the worked example and the 11 % ledger as the geophysical result. GRL manuscript kept as backup; text reused. Relation to Hoffman et al. (2025, JGR-MLC): companion — that paper located interannual SIE predictability in the ice; this one shows the ocean surface adds nothing at decadal scale. (Check the τ < T averaging-window alignment in that paper for the same overlap construction.)

**Attribution angles that do not need OHC (for the Discussion / future work, ranked):** (i) concurrent atmospheric attribution of the 89 % — regress the offset residual on summer Z500/Z200 or SLP over the trend decade (Ding's Greenland-high pathway); this is attribution, not prediction, and is the honest complement to the predictability null (Zach's item 9); (ii) *state* rather than trend targets at 1–3 yr lead with the SST map, i.e. does SST add to the JGR-MLC ice-only predictors (same pipeline, `trend_anom` → SIE anomaly); (iii) Pacific-sector September SIE as the target (Chukchi/Beaufort/E. Siberian) where the bridge should be strongest; (iv) short windows / RILEs (8.11). OHC (TEMP 0–300 m, Barents–Kara) stays the recommended follow-up predictor but is not attempted here.
...
**Status block (2026-09-15).**

*Runs.* `07 --tag off1` done (AUROC: SIE 0.612, Arctic SST 0.602, Niño 3.4 0.571, IPO 0.658, Pacific 0.662, indices 0.671, SIE+IPO 0.681, volume 0.651, SIE+vol+Pacific 0.686; F1 of every model ≈ always-positive 0.30 — at this skill level F1 cannot separate models, so AUROC + AUPRC (chance = prevalence 0.17) are the reported metrics). **On honest labels the IPO alone outranks the ice state.** `04_cesm2le_cnn_regress.py --tag off1_aux` (splits 2 5 7 × 2 seeds) finished — read `results/regression/off1_aux/regress_summary.md`. 8.12 thickness maps (SEP and MAR): SIE + 20 PCs ≤ 0.10, sectors ≤ 0.09 → the thickness *pattern* carries no decadal memory beyond the scalars; closed. Splits `off1` (1990–2029) built; `off1_aux` exists; full off1 CNN (base + warm-started aux) to launch with `LABELS=$LBL1 scripts/run_retrain.sh off1 off1_aux`, then `run_postprocess.sh off1 off1_aux`.

*Multi-method XAI (5.3b, `05b_xai_compare.py`, rel_aux split 2 seed 0).* LRP-z, LRP-α2β1, DeepTaylor and input×gradient agree: 22–31 % of |relevance| in the Arctic against a 4 % area share, Pacific at or below area share; occlusion for the same network: Arctic −0.077, Pacific ≤ +0.01. The two positive-only rules (α2β1, DeepTaylor) draw smooth bands along every ice edge and across the tropical Pacific; the two signed methods are speckle. Method agreement — the fix recommended by Mamalakis et al. 2022 and Bommer et al. 2024 — is satisfied and the agreed-upon region is a scalar proxy. Composite normalisation fixed (99th percentile, clipped). IG, SmoothGrad and SHAP did not run (messages lost — no log); rerun with `tee`, `pip install shap`.

*Literature (`docs/LITERATURE_COUPLING.md`).* The coupling is Oldham's problem / mathematical coupling (Oldham 1962; Tu & Gilthorpe 2007; Kenney 1982 *WRR*); the OLS-window form corr(y₀, slope) = −√(3(w−1)/(w(w+1))) (−0.71 at w = 2, 3; −0.63 at w = 5; −0.50 at w = 10; 0 once y₀ leaves the window) is not in the climate or ML literature. **LB22 §2.4 confirms the onset-inclusive window** ("OHC100 for the year 2000 → whether 2000–2009 will be a slowdown") with an IPO-logistic as the only baseline; the coupling enters via OHC100(t) ↔ GMST(t) (magnitude for GMST untested — optional one-script check). PROCAST and Hoffman et al. 2025 predict *states* with a persistence null → no arithmetic coupling (the τ < T cells there share years between input and target averages, which shortens the effective lead but does not inflate skill relative to persistence). DelSole & Tippett 2018 supply the language: group demeaning removes forced predictability, the offset removes the arithmetic, the ledger is the initial-value predictability of the residual. Positioning: derive and quantify a known statistical effect inside a widely used label; do not claim "nobody noticed".

*Reframing decision.* Lauren: "you need skill before XAI" is not a publishable result. Agreed. Headline order for the AIES paper: (1) the coupling, derived (App. A) and measured (0.28 → 0.075 → 0.04; JJA SST is a 1–3-month proxy for Sept SIE(t), so a JJA → Sept-onward trend forecast is a nowcast of this year's ice with a decadal label — "forecast" is fine, "precursor" is not); (2) seven attribution methods agree and are all describing the proxy — occlusion beside relevance is mandatory, not optional; (3) the geophysical ledger (11 %; IPO ≥ ice extent on honest labels; the Pacific bridge with the model's sign). The checklist becomes one paragraph. Target change (residual-SIE state at 1–3 yr) rejected: it re-enters Hoffman 2025's territory, where SST's +0.02 R² over ice persistence is a footnote.

*Observations restored (6.3/6.4, `scripts/10_obs_baseline_predict.py`).* The v1 obs forecast was dropped because it rested on the onset-inclusive CNN (i.e. it restated the 2025 ice state). With offset labels it returns as Fig. 8b: logistic SIE / IPO / SIE+IPO / SIE+Pacific fitted on CESM2-LE (9 split-fits → band), applied to the observed Sept SIE anomaly (NSIDC, forced reference removed exactly as for the CNN input: ensmean / group_cmip6 / group_smbb / linear) and ERSSTv5 JJA IPO and Niño 3.4; the observed offset-window z is plotted where the decade is complete (onsets ≤ 2015) so the reader sees hits and misses; the off1 CNN vote fraction is overlaid once postprocess has run. Caveats for the caption: vote fractions are not calibrated probabilities (Weisheimer & Palmer 2014); the IPO term inherits CESM2's opposite-sign bridge; observed IPO is the ERSST filtered index, the model's is per-member internal — same units, different detrending. Outputs `results/obs_predict/w10_off1/`, figure `diagnostics/obs_predict_w10_off1_<forced>.png`.
