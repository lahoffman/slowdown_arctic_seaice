...
def plot_ledger(delta: dict, corr_map, lat, lon, out_png, base_r2: float, state_label: str = "ice state") -> None:
    """(a) added test R² over the target's state per OHC predictor set; (b) correlation of the residual with OHC at onset."""
    ...
    ax.axvline(0, color=st.INK, lw=0.6); ax.set_xlabel(f"added test R² beyond the {state_label} (R² {base_r2:.3f})")
