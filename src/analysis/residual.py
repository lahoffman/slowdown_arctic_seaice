...
def residual_skill(trend_anom: np.ndarray, sie_anom: np.ndarray, idx_onset: Dict[str, np.ndarray],
                   idx_conc: Dict[str, np.ndarray], years: np.ndarray, n_splits: int = 9) -> xr.Dataset:
    """
    Per split: test R² of trend ~ SIE(t) (stage 1) and of residual ~ index set (stage 2),
    ...
    fields = {"trend": trend_anom, "sie_anom": sie_anom,
    ...
    for k, te_b, va_b, tr_b in bl.iter_split_blocks(n_splits, 10):
        ...
        s_tr, s_te = d["tr"]["sie_anom"][:, None], d["te"]["sie_anom"][:, None]
...
def pooled_residual(trend_anom: np.ndarray, sie_anom: np.ndarray) -> Tuple[np.ndarray, float]:
    """Residual of trend ~ SIE(t) fit on all member-years (descriptive maps only); returns (resid, r²)."""
    y, x = trend_anom.ravel(), sie_anom.ravel()
    ok = np.isfinite(y) & np.isfinite(x)
    fit, _, _ = _ols(x[ok, None], y[ok], x[ok, None])
...
def summary_markdown(ds: xr.Dataset, r2_pooled: float, n_events: Optional[int] = None) -> str:
    ...
    lines = [f"# Residual analysis — trend anomaly beyond the ice state\n",
             f"Stage 1: trend anomaly ~ September SIE anomaly at onset. Test R² median over 9 splits "
